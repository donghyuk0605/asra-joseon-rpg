...
describe('mobile combat HUD layout', () => {
  ...
  it('keeps the portrait playfield center clear and edge-mounts landscape controls', () => {
    expect(styles).toMatch(/max-width:\s*700px\)[\s\S]*orientation:\s*portrait[\s\S]*\.quest-chip,[\s\S]*\.momentum-hud\s*\{\s*display:\s*none/);
