...
describe('story chronicle', () => {
  ...
  it('gives Crown Prince Gwanghae a separate bunjo chronicle under King Seonjo', () => {
    ...
    expect(hud).toContain("chapters: '10장'");
  ...
  it('records viewed scenes and irreversible choices, with replay from the journal', () => {
    expect(hud).toContain('data-action="story-replay"');
    expect(hud).toContain('data-id="story-memories"');
    expect(hud).toContain('data-id="story-choices"');
    expect(hud).toContain('renderStoryHistory(snapshot.storyState)');
    expect(scene).toContain('new StoryDirector');
    expect(scene).toContain('completeStoryBeat(');
    expect(scene).toContain('this.checkpointSinglePlayer()');
    expect(styles).toContain('.story-cinematic__choices');
    expect(styles).toContain('.story-memory-log');
  });
});
