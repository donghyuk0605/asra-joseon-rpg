...
describe('HUD pointer safety', () => {
  ...
  it('lets the visible story objective open the chronicle without leaking the click to the world', () => {
    expect(styles).toMatch(/\.quest-chip\[data-action="story-journal"\][^}]*pointer-events:\s*auto/);
  });
});
