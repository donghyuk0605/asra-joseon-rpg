...
const MONSTER_ROLE_LABELS: Record<MonsterKind, string> = {
  ...
  'japanese-shogun': '오사카 막부 · 검은 부채의 쇼군',
...
const CHARACTER_STORY_PROFILES: Record<PlayerOrigin, CharacterStoryProfile> = {
  'kim-donghyeok': {
    ...
    acts: [
      { title: '제1막 · 탈옥', chapters: '1–5장', premise: '울릉 수탈의 증거를 좇는다.', turningPoint: '서병관의 왜구 밀약을 밝히고 섬을 해방한다.' },
      { title: '제2막 · 봉기', chapters: '6–11장', premise: '본토 관아와 왜군 보급망을 끊는다.', turningPoint: '복수를 개인의 원한이 아닌 국정의 죄로 바꾼다.' },
      { title: '제3막 · 대동', chapters: '12–17장', premise: '북방 전란과 무너진 왕도를 돌파한다.', turningPoint: '복수 뒤 어떤 나라를 남길지 결정한다.' },
    ],
  },
  'frontier-archer': {
    ...
    acts: [
      { title: '제1막 · 회맹', chapters: '1–7장', premise: '패전 뒤 장백산 세 부족의 신뢰를 얻는다.', turningPoint: '흑수부 대회맹으로 여진군을 하나로 묶는다.' },
      { title: '제2막 · 설욕', chapters: '8–11장', premise: '압록과 평양의 성곽을 차례로 돌파한다.', turningPoint: '대동관에 검은 깃발을 세워 한성 북로를 연다.' },
      { title: '제3막 · 새 깃발', chapters: '12–15장', premise: '조선 왕실과 자신의 출생을 마주한다.', turningPoint: '정복과 연맹 중 군세가 따를 미래를 정한다.' },
    ],
  },
  'osaka-mudang': {
    journalTitle: '연화의 망향원혼록',
    epithet: '이름 잃은 포로들의 망향 무당',
    faction: '망향민 원혼선단',
    homeland: '오사카 포로촌 · 조선인 징발선의 종착지',
    era: '아스라 세계선 · 일본 출병항에서 조선 왕도까지 이어진 포로의 길',
    premise: '조선과 일본 양쪽에 버림받은 무당이 포로의 이름과 원혼을 모아 두 나라의 전쟁 장부를 뒤엎는다.',
    creed: '기록되지 않은 죽음도 이름을 불러 주면 증언이 된다.',
    wound: '고향은 송환을 외면했고 타향은 이름과 가족을 빼앗았다.',
    adversary: '포로 무역으로 이득을 본 쇼군·다이묘·조선 관리와 연화를 삼키려는 원귀',
    allies: '포로촌 생존자 · 일본인 징발 피해자 · 원혼 선단 · 전쟁 고아',
    ...
    acts: [
      { title: '제1막 · 탈주굿', chapters: '1–5장', premise: '포로촌에서 천수각까지 침공 명령을 거슬러 오른다.', turningPoint: '쇼군을 꺾고 조선으로 가는 출병선을 빼앗는다.' },
      { title: '제2막 · 바닷길', chapters: '6–12장', premise: '포로 운송망을 역으로 끊는다.', turningPoint: '부산진에 귀향자가 아닌 심판자로 상륙한다.' },
      { title: '제3막 · 이름굿', chapters: '13–24장', premise: '탄금대에서 압록까지 국가의 책임을 추적한다.', turningPoint: '복수를 끝내고 이름 잃은 이들의 새 고향을 세운다.' },
    ],
  },
  'gwanghae-prince': {
    ...
    acts: [
      { title: '제1막 · 남겨진 조정', chapters: '1–4장', premise: '몽진한 선조 뒤에서 백성과 관군을 수습한다.', turningPoint: '왕명이 아닌 민심의 군대를 만든다.' },
      { title: '제2막 · 일곱 고을', chapters: '5–9장', premise: '둔전·수운·의병 명부를 이어 분조를 완성한다.', turningPoint: '평양 대동문에서 왕세자가 직접 전선을 지킨다.' },
      { title: '제3막 · 왕좌와 왕명', chapters: '10–11장', premise: '선조에게 분조 장계를 내민다.', turningPoint: '쿠데타 또는 의병 진압의 결과를 감당한다.' },
    ],
  },
};
...
const MUDANG_STORY_CHAPTERS = [
  ...
  ['검은 부채의 쇼군', '천수각 친위대와 쇼군을 꺾고 조선 침공 명령서와 출병선을 빼앗다'],
...
export class Hud {
  ...
  setTravelMode(enabled: boolean): void {
    ...
    this.text(
      ...
      enabled
        ...
        : '군사 거점의 공방전과 한성·조선 명읍의 역참길을 함께 살핍니다.',
    );
    this.text(
      'world-map-status',
      enabled
  ...
  private configureCombatSkillLoadout(origin: PlayerOrigin, bowEquipped: boolean): void {
    ...
    if (intro) intro.textContent = osakaMudang
      ...
      : frontierArcher
        ? frontierBow
        ? '활 장착: 자동 추적 다발시 · 강궁 관통시 · 화살비를 Q · W · E · R로 발동합니다.'
        : '환도 장착: Q는 회전베기. 다른 검술은 수련·전수·비급으로 익히며, 활을 다시 들면 신궁술로 즉시 전환됩니다.'
      : '기초 수련 · 장인 전수 · 비급 습득 · 사건 각성으로 익힙니다. Q · W · E · R로 발동합니다.';
  }
  ...
  private template(): string {
    ...
    const settlementMapPins = WORLD_MAP_NODES
      .filter((node) => worldMapNodeKind(node) === 'settlement' && !('landmarkFrame' in node))
      .map((node) => `
      <button
        class="world-map-place-pin"
        data-world-region="${node.destination}"
    ...
    const settlementNodes = WORLD_MAP_NODES
      ...
      .map((node) => {
        ...
        >
          ...
          <em aria-label="고을 편의 시설"><strong>역참</strong><strong>장시</strong><strong>안전지대</strong></em>
          <small class="world-map-access-state" data-world-node-state>미발견</small>
        </button>`;
      }).join('');
    ...
    return `
      ...
      <aside class="field-minimap" aria-label="현재 지역 미니맵">
        ...
        <div class="minimap-surface" data-id="minimap-surface">
          <i class="minimap-player" aria-label="김동혁 현재 위치"></i>
      ...
      <section class="player-panel ornate-panel">
        <div class="portrait" aria-label="김동혁 초상"><img class="player-portrait-image" src="/assets/ui/kim-donghyeok-portrait-v1.png" alt=""><i></i></div>
        <div class="player-info">
          <span class="eyebrow"><span data-id="player-affiliation">청해진 토벌대</span> · <em data-id="player-kit">복장 미착용 · 맨발</em></span>
          <strong class="player-name" data-id="player-name">김동혁</strong>
      ...
      <section class="story-journal-panel" id="story-journal-panel" role="dialog" aria-modal="true" aria-labelledby="story-journal-title" aria-hidden="true" inert>
        <header>
          ...
          <div><span>ASRA CHRONICLE</span><strong id="story-journal-title">김동혁의 복수록</strong></div>
        ...
        <section class="story-acts" aria-label="삼막 이야기 구조">
          <header><span>三幕 · STORY ARC</span><b>운명의 세 막</b></header>
      ...
      <section class="world-map-panel" id="world-map-panel" role="dialog" aria-modal="true" aria-labelledby="world-map-title" aria-hidden="true" inert>
              ...
              <section class="war-council-panel" aria-label="플레이어 세력 전황">
                <div class="world-map-tab-panel world-map-settlements-panel" data-world-map-tab-panel="settlements">
                  <nav class="world-map-settlement-index" aria-label="한성과 조선 명읍 역참">
                    <header><span>驛路 · SAFE SETTLEMENTS</span><b>한성·조선 명읍</b><small>역참길과 장시</small></header>
        ...
        <footer>
          ...
          <b data-id="world-map-status">성곽은 전황을, 고을은 역참·장시와 안전지대를 표시합니다.</b>
        </footer>
      </section>
      ...
      <section class="inventory-panel" id="inventory-panel" data-mobile-tab="bag" role="dialog" aria-modal="true" aria-labelledby="inventory-title" aria-hidden="true" inert>
        <header class="inventory-titlebar">
          <div class="inventory-heading"><span>CHARACTER · INVENTORY</span><strong id="inventory-title">김동혁의 행낭</strong></div>
