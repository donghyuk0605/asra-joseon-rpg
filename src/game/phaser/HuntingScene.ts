...
import { REGIONS, type JapanRegionId, type JoseonTownRegionId, type RegionId } from '../world/regions';
...
export class HuntingScene extends Phaser.Scene {
  ...
  startFrontierArcherStory(): void {
    ...
    const x = this.simulation.player.x;
    const y = this.simulation.player.y - 126;
    this.alertMarker(x, y, '하진: “압록에서 졌다. 이제 흩어진 부족부터 하나로 묶는다.”', 2850);
    this.time.delayedCall(2600, () => {
      this.alertMarker(x, y, '여진 우두머리: “네 활로 남쪽 문을 열어라. 우리는 함께 내려간다.”', 2500);
    });
    this.time.delayedCall(5350, () => {
      this.alertMarker(x, y, '하진: “버린 피가 무엇을 무너뜨리는지 보게 해주마.”', 2500);
    });
  ...
  startOsakaMudangStory(): void {
    ...
    const x = this.simulation.player.x;
    const y = this.simulation.player.y - 132;
    this.alertMarker(x, y, '연화: “조선은 우리를 포기했고, 오사카는 우리 이름을 빼앗았다.”', 2700);
    this.time.delayedCall(2950, () => {
      this.alertMarker(x, y, '포로촌 원혼: “출병문을 열어라. 산 자의 배를 타고 고향으로 돌아가자.”', 2850);
    });
    this.time.delayedCall(6000, () => {
      this.alertMarker(x, y, '연화: “돌아가는 것이 아니다. 버린 나라의 심장을 받으러 간다.”', 2800);
    });
  }
  ...
  create(): void {
    ...
    const skillKeys = ['keydown-Q', 'keydown-W', 'keydown-E', 'keydown-R'];
    for (const [index, key] of skillKeys.entries()) this.input.keyboard?.on(key, () => {
      ...
      if (!this.isGameplayInputLocked() && this.gameMode !== 'travel') this.simulation.castSkill(skillId);
    });
    if (import.meta.env.DEV) {
  ...
  update(_: number, delta: number): void {
    ...
    if (this.prologueActive || this.gwanghaeChoiceDom !== null || this.royalRefugeDom !== null) return;
    if (this.canWriteSinglePlayerSave()) {
  ...
  private finishOpeningPrologue(): void {
    ...
    this.prologueTimers.push(this.time.delayedCall(2450, () => this.simulation.startPrisonAmbush()));
  }
  ...
  private createCampaignWorld(): void {
    ...
    routePlaque('osaka', 768, 145, '셋쓰 내륙문 · 감시대 격파 후 개방', 'settsuvillage', 'south', true);
    routePlaque('settsuvillage', 768, 890, '남쪽 포로길 · 오사카 외항', 'osaka', 'north');
    routePlaque('settsuvillage', 768, 145, '야마자키 산길 · 징발대 격파 후 개방', 'yamazakihunt', 'south', true);
    routePlaque('yamazakihunt', 768, 890, '남쪽 산촌길 · 셋쓰 산촌', 'settsuvillage', 'north');
    routePlaque('yamazakihunt', 768, 145, '오사카 성로 · 사냥숲 돌파 후 개방', 'osakacastle', 'south', true);
    routePlaque('osakacastle', 768, 890, '남쪽 삼나무길 · 야마자키', 'yamazakihunt', 'north');
    routePlaque('osakacastle', 768, 145, '천수각 외문 · 성하 수비대 격파 후 개방', 'shogunkeep', 'south', true);
    routePlaque('shogunkeep', 768, 890, '남쪽 성하길 · 오사카 성하', 'osakacastle', 'north');
    routePlaque('shogunkeep', 768, 145, '서남 해로 · 쇼군 격파 후 사카이', 'sakaicity', 'south', true);
    routePlaque('sakaicity', 768, 890, '북쪽 성채길 · 오사카 쇼군 성채', 'shogunkeep', 'north');
    routePlaque('sakaicity', 768, 145, '이즈미 고개 · 용병대 격파 후 개방', 'izumihunt', 'south', true);
    routePlaque('izumihunt', 768, 890, '북쪽 상인길 · 사카이 자유항', 'sakaicity', 'north');
    routePlaque('izumihunt', 768, 145, '아와지행 나루 · 매복대 격파 후 승선', 'awajicoast', 'south', true);
    routePlaque('awajicoast', 768, 890, '혼슈행 선착장 · 이즈미 귀환선', 'izumihunt', 'north');
    routePlaque('awajicoast', 768, 145, '이키행 선착장 · 왜구 초소 격파 후 승선', 'ikiport', 'south', true);
    routePlaque('ikiport', 768, 890, '아와지행 나루 · 세토 내해 귀환선', 'awajicoast', 'north');
    routePlaque('ikiport', 768, 145, '대마도행 선착장 · 보급대 격파 후 승선', 'tsushimahunt', 'south', true);
    routePlaque('tsushimahunt', 768, 890, '이키행 나루 · 고노우라 귀환선', 'ikiport', 'north');
    routePlaque('tsushimahunt', 768, 145, '이즈하라 산길 · 산림대 격파 후 개방', 'izuhara', 'south', true);
    routePlaque('izuhara', 768, 890, '북쪽 산길 · 아리아케 산림', 'tsushimahunt', 'north');
    routePlaque('izuhara', 768, 145, '조선 해협 · 도주군 격파 후 부산진', 'busanjin', 'south', true);
    routePlaque('jeonju', 112, 510, '서문 역참 · 부산진성 출정', 'busanjin', 'south');
    routePlaque('busanjin', 768, 145, '북문 군로 · 탄금대', 'tangeumdae', 'south');
    routePlaque('tangeumdae', 768, 890, '남행 군로 · 부산진성', 'busanjin', 'north');
    routePlaque('tangeumdae', 768, 150, '한성 파발로 · 왜군 전멸 후 개방', 'gyeongbokgate', 'south');
    routePlaque('gyeongbokgate', 768, 880, '남쪽 군로 · 탄금대', 'tangeumdae', 'north');
    routePlaque('gyeongbokgate', 768, 145, '흥례문 · 근정전', 'gyeongbokcourt', 'south');
    routePlaque('gyeongbokcourt', 768, 890, '금천교 · 광화문', 'gyeongbokgate', 'north');
    routePlaque('gyeongbokcourt', 768, 140, '사정문 · 왕의 내전', 'gyeongbokinner', 'south');
    routePlaque('gyeongbokinner', 768, 890, '근정전 회랑', 'gyeongbokcourt', 'north');
    routePlaque('gyeongbokinner', 1260, 520, '북방 군보 · 평양 내성', 'pyongyanginner', 'south');
    routePlaque('jurchenvillage', 768, 145, '북행 자작나무길 · 여진 통합 시작', 'changbaihunt', 'south');
    routePlaque('jurchenvillage', 768, 890, '남쪽 목책문 · 세 부족 통합 후 압록 설욕전', 'manchufrontier', 'north');
    ...
    routePlaque('manchufrontier', 768, 145, '북행 설원길 · 여진 부락', 'jurchenvillage', 'south');
    routePlaque('manchufrontier', 768, 890, '남진 성문 · 평양 외성', 'pyongyangouter', 'north');
    routePlaque('pyongyangouter', 768, 145, '북행 설원길 · 압록 국경', 'manchufrontier', 'south');
    routePlaque('pyongyangouter', 768, 890, '외성 남문 · 대동문', 'pyongyanggate', 'north', true);
    routePlaque('pyongyanggate', 768, 145, '북곽 군로 · 평양 외성', 'pyongyangouter', 'south');
    routePlaque('pyongyanggate', 768, 890, '대동문 안길 · 평양 내성', 'pyongyanginner', 'north', true);
    routePlaque('pyongyanginner', 768, 145, '대동문 회랑', 'pyongyanggate', 'south');
    routePlaque('pyongyanginner', 768, 890, '한성 북로 · 경복궁 광화문', 'gyeongbokgate', 'north', true);
  ...
  private createJapaneseVillageLife(): void {
    ...
    for (const resident of [
      ...
      },
      {
        ...
        y: osaka.y + 590,
        name: '염색공 유키',
        dialogue: '당신이 포로촌을 빠져나온 조선 무녀군요. 군사와 백성을 가려 주길 바라요.',
        mode: 'japanese-civilian' as const,
        tint: 0xdce7ef,
        speed: 16,
        patrol: [
      ...
      },
      {
        ...
        y: castle.y + 690,
        name: '성하 피난민 사요',
        dialogue: '쇼군 친위대는 성문 안에서 두 번 진형을 바꿉니다. 조총 불빛을 먼저 피하세요.',
        mode: 'japanese-civilian' as const,
        tint: 0xe8d9df,
        speed: 12,
  ...
  private handleEvent(event: GameEvent): void {
    ...
    if (event.type === 'shogun-phase-changed') {
      ...
      if (monster) {
        ...
        this.alertMarker(monster.x, monster.y - 138, '쇼군 2단계 · “전군, 목숨을 바쳐 길을 막아라!”', 3300);
    ...
    if (event.type === 'shogun-defeated') {
      ...
      this.alertMarker(
        this.simulation.player.x,
        this.simulation.player.y - 150,
        `쇼군 격파 · ${event.gold}전 · 무공 점수 ${event.skillPoints} · 강화 주문서 전리품`,
        4200,
      );
    }
