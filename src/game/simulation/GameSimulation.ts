...
const MONSTER_DATA: Record<MonsterKind, { name: string; hp: number; damage: number; level: number }> = {
  ...
  'japanese-shogun': { name: '오사카 쇼군 아시카가 카게노부', hp: 1080, damage: 32, level: 18 },
...
const TIMID_ANIMAL_KINDS: ReadonlySet<MonsterKind> = new Set([
  'ulleung-hare', 'ulleung-water-deer', 'japanese-sika-deer',
]);
const CHARGER_KINDS: ReadonlySet<MonsterKind> = new Set([
  'boar', 'japanese-wild-boar', 'ulleung-sangun', 'korean-gray-wolf', 'manchu-cavalry',
]);
...
const tacticalRoleFor = (kind: MonsterKind): MonsterTacticalRole => {
  ...
  if (isRangedSoldier(kind) || kind === 'bandit' || kind === 'moon-revenant') return 'ranged';
  if (isSpearman(kind)) return 'spearman';
  if (kind === 'mine-golem' || kind === 'ulleung-executioner') return 'brute';
  return 'melee';
};
...
export class GameSimulation {
  ...
  private updateRegionFromPosition(): void {
    ...
    if (isJapanRegion(this.region) || isJurchenRegion(this.region)
      ...
      || isJoseonTownRegion(this.region)) next = this.region;
  ...
  private updateMonster(monster: MonsterState, dt: number): void {
    ...
    const aggroRange = monster.kind === 'japanese-shogun'
      ...
      : isRangedSkirmisher ? 245
        : monster.kind === 'boar' || monster.kind === 'japanese-wild-boar' ? 220
          : monster.kind === 'mine-golem' ? 190 : 205;
    ...
    else if (monster.kind === 'boar' || monster.kind === 'japanese-wild-boar') this.updateBoarAi(monster, distance, dt);
    else if (monster.kind === 'ulleung-sangun') this.updateTigerAi(monster, distance, dt);
  ...
  private updateTimidAnimalAi(monster: MonsterState, distance: number, dt: number): void {
    ...
    const threatened = monster.aggro || (this.playerActive && distance < 150);
    if (!threatened) {
      this.updatePatrol(monster, dt);
      return;
    }
    const angleAway = Math.atan2(monster.y - this.player.y, monster.x - this.player.x);
  ...
  private updateSkirmisherAi(monster: MonsterState, distance: number, dt: number): void {
    ...
    if (distance > preferredRange + 16) {
      ...
      this.moveMonsterToward(
        ...
        isRangedSkirmisher ? 74 : monster.kind === 'mine-golem' ? 58 : 82,
        dt,
        12,
      );
    } else {
  ...
  private performMonsterAttack(
    ...
    const impactRange = impactRangeOverride ?? (monster.kind === 'japanese-shogun' ? 176
      ...
      : monster.kind === 'bandit' || monster.kind === 'moon-revenant' ? 118
        : monster.kind === 'boar' || monster.kind === 'japanese-wild-boar' ? 98
